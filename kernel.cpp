...
__global__ void
gpu_bsw::sequence_dna_kernel_long_reads(char* seqA_array, char* seqB_array, unsigned* prefix_lengthA,
                    unsigned* prefix_lengthB, short* seqA_align_begin, short* seqA_align_end,
                    short* seqB_align_begin, short* seqB_align_end, short* top_scores, short* d_h_curr, short* d_h_prev, short* d_h_prev_prev, short* d_e_curr, short* d_e_prev, short* d_e_prev_prev,short* d_f_curr, short* d_f_prev, short* d_f_prev_prev, short matchScore, short misMatchScore, short startGap, short extendGap)
{
  ...
  //  if(thread_Id == 0)printf("total_threads:%d\nm",total_threads);
    ...
    char*    seqA;
    char*    seqB;
    short *h_curr, *h_prev, *h_prev_prev, *e_curr, *e_prev, *e_prev_prev, *f_curr, *f_prev, *f_prev_prev;
...
// setting up block local sequences and their lengths.
    if(block_Id == 0)
    {
        ...
        h_curr = d_h_curr;
        h_prev = d_h_prev;
        h_prev_prev = d_h_prev_prev;
        e_curr = d_e_curr;
        e_prev = d_e_prev;
        e_prev_prev = d_e_prev_prev;
        f_curr = d_f_curr;
        f_prev = d_f_prev;
        f_prev_prev = d_f_prev_prev;
...
memset(h_prev_prev, 0, (minSize+1) * sizeof(short));
    ...
    {  // iterate for the number of anti-diagonals
      ...
      //  is_valid = is_valid - (diag < minSize || diag >= maxSize); //move the pointer to left by 1 if cnd true
        ...
        int _diag_id = diag + 1;
        if (_diag_id < minSize){
          diagonal_length = _diag_id;
        }else if (maxSize < _diag_id){
          diagonal_length = maxSize + minSize - _diag_id;
        }else{
